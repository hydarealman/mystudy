...
void PWM_Init(void)
{
	...
	GPIO_InitStructures.GPIO_Pin = GPIO_Pin_2;			// 选择pa0引脚
	...
	TIM_TimeBaseInitStructures.TIM_Prescaler = 36 - 1;					 // 预分频系数
	...
	TIM_OC3Init(TIM2,&TIM_OCInitStructures);	// 初始化通道1
...
{
	TIM_SetCompare3(TIM2,Compare); // 设置通道1的比较值
}
